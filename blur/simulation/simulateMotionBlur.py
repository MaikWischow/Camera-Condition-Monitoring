...
def generateMotionBlurKernel(kernelSaveDir, kernelSize, intensity, asNumpyArray=False):
    """
    Generates a motion blur kernel.
    :param kernelSaveDir: Directory where to save thegenerated kernel as an image.
    :param kernelSize: Overall size of the kernel image in px (this is not the size of the actual kernel!).
    :param intensity: Intensity parameter in range [0,1] (0==linear kernel, 1==non-linear kernel)
    :param asNumpyArray: Return the motion blur kernel as a numpy array.
    :return: Generate motion blur kernel wrapped in the above kernel class (default) or as numpy array (if asNumpyArray==True).
    """
    
    if not 0 <= intensity <= 1:
        raise Exception("Error: Intensity parameter is", intensity, "and not 0 <= kernelSize <= 1.")
    if kernelSize % 2 == 0:
        kernelSize += 1
    
    goodKernelSize = False
    kernel = None
    if kernelSize >= 7:
        while(not goodKernelSize):
            kernel = Kernel(size=(kernelSize, kernelSize), intensity=intensity)
            
            # Heuristic to get faster to the desired kernel sizes 
            # (since there is no parameter for the actual path length of the kernel in px).
            # The desired kernel is selected manually afterwards.
            if kernelSize <= 10 or intensity <= 0.3:
                nonLinFactor = 1.1 
            elif kernelSize <= 15 or intensity <= 0.5:
                nonLinFactor = 1.4
            else:
                nonLinFactor = 1.6
            goodKernelSize = np.max(kernel.kernelMatrix) <= (1.0 / (nonLinFactor * kernelSize))
            print(goodKernelSize)
    else:
        kernel = Kernel(size=(kernelSize, kernelSize), intensity=intensity)
    
    if kernelSaveDir is not None:
        kernel.displayKernel(save_to=os.path.join(kernelSaveDir, "motionBlurKernel.png"))
    return kernel.kernelMatrix if asNumpyArray else kernel

def addMotionBlur(img, kernel):
    """
    Apply a kernel to an image.
    :param img: Input image to blur.
    :param kernel: Motion blur kernel.
    :return: Motion blurred image.
    """
    
    if isinstance(kernel, Kernel):
        blurred = kernel.applyTo(img, keep_image_dim=True)
        if len(blurred.getbands()) > 1:
           blurred = np.array(blurred.convert('L'))
    else:
        blurred = np.array(convolve(img, kernel, mode="same").astype("uint8"))
...
#     motionBlurSize = 11
#     dirOut = os.path.join(r"../../data/udacity/img/motionBlur/linear/", str(motionBlurSize))
    
#     if not os.path.exists(dirOut):
#         os.makedirs(dirOut)
        
#     kernelImg = cv2.imread(os.path.join(r"../../data/kernels/motionBlur/linear/", str(motionBlurSize), str(motionBlurSize) + ".png"))
#     kernelImg = cv2.cvtColor(kernelImg, cv2.COLOR_RGB2GRAY)
#     kernelImg = kernelImg.astype("float32") / np.sum(kernelImg)
#     img = cv2.imread(os.path.join(dirIn, imgName))
#     img = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)
#     motionBlurredKernel = addMotionBlur(img, kernelImg)
#     cv2.imwrite(os.path.join(dirOut, imgName), motionBlurredKernel)