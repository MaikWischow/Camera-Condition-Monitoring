function [kernel, interim_latent] = blind_deconv(y, lambda, lambda_grad, opts,patch_r)
% 
...
if ~exist('patch_r','var')
    opts.r = floor(0.025*mean(size(y)));
else
    opts.r = patch_r;
end
...
for s = num_scales:-1:1
  ...
  %-----------------------------------------------------------%
  opts.s = s;
  [ks, lambda, lambda_grad, interim_latent] = blind_deconv_main(ys, ks, lambda,...
